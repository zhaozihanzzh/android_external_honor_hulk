...
enum seapp_debug_log {
    ...
    SEAPP_CONTEXT_ALLOC_MEMORY_FILE_OPEN_SUCCESS = 1<<5,
    SEAPP_CONTEXT_GET_LABEL_FILE_OPEN_ERROR = 1<<6,
    SEAPP_CONTEXT_GET_LABEL_FILE_OPEN_SUCCESS = 1<<7,
    SEAPP_CONTEXT_ALLOC_MEMORY_FILE_READ_NORMAL = 1<<8,
    SEAPP_CONTEXT_ALLOC_MEMORY_FILE_READ_SUCCESS= 1<<9,
    SEAPP_CONTEXT_GET_LABEL_FILE_READ_NORMAL = 1<<10,
    SEAPP_CONTEXT_GET_LABEL_FILE_READ_SUCCESS = 1<<11,
    SEAPP_CONTEXT_ALLOC_MEMORY_FILE_UPDATE_NUM = 1<<12,
    SEAPP_CONTEXT_GET_LABEL_FILE_UPDATE_NUM = 1<<13,
    SEAPP_CONTEXT_GET_LABEL_FILE_SORT = 1<<14,
    SEAPP_CONTEXT_GET_LABEL_FILE_FREE = 1<<15
};
...
static void free_seapp_contexts(void)
{
	...
	seapp_contexts = NULL;
	nspec = 0;
	seapp_log_flag |= SEAPP_CONTEXT_GET_LABEL_FILE_FREE;
}
...
int selinux_android_seapp_context_reload(void)
{
	...
	for (i = 0; i < files_len; i++) {
		...
		seapp_log_flag |= SEAPP_CONTEXT_ALLOC_MEMORY_FILE_OPEN_SUCCESS;
		while (fgets(line_buf, sizeof line_buf - 1, fp)) {
			seapp_log_flag |= SEAPP_CONTEXT_ALLOC_MEMORY_FILE_READ_NORMAL;
			p = line_buf;
			while (isspace(*p))
				p++;
			if (*p == '#' || *p == 0)
				continue;
			seapp_log_flag |= SEAPP_CONTEXT_ALLOC_MEMORY_FILE_UPDATE_NUM;
			nspec++;
		}
		fclose(fp);
	}

	seapp_log_flag |= SEAPP_CONTEXT_ALLOC_MEMORY_FILE_READ_SUCCESS;
	...
	for (i = 0; i < files_len; i++) {
		...
		seapp_log_flag |= SEAPP_CONTEXT_GET_LABEL_FILE_OPEN_SUCCESS;
		while (fgets(line_buf, sizeof line_buf - 1, fp)) {
			seapp_log_flag |= SEAPP_CONTEXT_GET_LABEL_FILE_READ_NORMAL;
			len = strlen(line_buf);
			if (line_buf[len - 1] == '\n')
				line_buf[len - 1] = 0;
			p = line_buf;
			...
			seapp_log_flag |= SEAPP_CONTEXT_GET_LABEL_FILE_UPDATE_NUM;
	...
	seapp_log_flag |= SEAPP_CONTEXT_GET_LABEL_FILE_READ_SUCCESS;
	qsort(seapp_contexts, nspec, sizeof(struct seapp_context *),
	      seapp_context_cmp);

	seapp_log_flag |= SEAPP_CONTEXT_GET_LABEL_FILE_SORT;
