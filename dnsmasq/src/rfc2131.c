...
static void change_mac(char *input);
static int symbol_logical(char *for_check, size_t i);
static int char_hexadecimal(char *for_check);
static void change_ip(char *ip_input);
...
void change_mac(char *mac_input)
{
  ...
  /* the MAC adress xx:xx:xx:xx:xx:xx len is 17, if the length less than 17 , the inupt is no MAC address */
  if (len < 17) {
    return ;
  }

  /* traverse strings, find MAC adress */
  size_t i = 2;
...
int symbol_logical(char *for_check, size_t i)
{
  if ((':' == *(for_check + i)) && (':' == *(for_check + i + 3)) &&
    (':' == *(for_check + i + 6)) && (':' == *(for_check + i + 9)) && (':' == *(for_check + i + 12))) {
    return 1;
  } else {
    return 0;
  }
...
int char_hexadecimal(char *for_check)
{
  if ((('0' <= *for_check) && ('9' >= *for_check)) || (('a' <= *for_check) &&
    ('f' >= *for_check)) || (('A' <= *for_check) && ('F' >= *for_check))) {
    return 1;
  } else {
    return 0;
  }
